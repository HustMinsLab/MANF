...
def load_data():
    ...
    dev_data = []
    test_data = []
    with open('dataset/train.tsv', 'r', encoding='utf-8') as tsvtrain:
        train_file_list = csv.reader(tsvtrain,'mydialect')
        for line in train_file_list:
            train_data.append(line)
            
    with open('dataset/train.tsvdev.tsv', 'r', encoding='utf-8') as tsvdev:
        dev_train_file_list = csv.reader(tsvdev,'mydialect')
        for line in dev_train_file_list:
            dev_data.append(line)
            
    with open('dataset/train.tsvtest.tsv', 'r', encoding='utf-8') as tsvtest:
